...
local menus2 = {
	...
	window.is_window_settings = is_window_settings
		...
		function window:create_box_no_scroll (n)
			local container = CreateFrame ("Frame", "DetailsOptionsWindow" .. n, window.widget,"BackdropTemplate")
		...
		function window:create_box (n)
			local container_window = CreateFrame ("ScrollFrame", "Details_Options_ContainerScroll" .. n, window.widget,"BackdropTemplate")
			local container_slave = CreateFrame ("Frame", "DetailsOptionsWindow" .. n, container_window,"BackdropTemplate")
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		function window:create_line_background2 (frameX, label, parent, icon, is_button1, is_button2)
			local f = CreateFrame ("frame", nil, frameX,"BackdropTemplate")
			f:SetPoint ("left", label.widget or label, "left", -2, 0)
			f:SetSize (260, 16)
...
-------------------------------------------------------------------------------------------------------------------	
	...
	--SKINS ~skins ~elvui
		function window:UseElvUISkin()
			...
			--> title bar
			local titlebar = CreateFrame ("frame", window:GetName() .. "OptionsTitleBar", window_widget,"BackdropTemplate")
...
function window:CreateFrame17()
	...
	--> combat alpha modifier
		...
		for id, i in ipairs(optionsOrder) do
			local line = _G.CreateFrame("frame", nil, frame17,"BackdropTemplate")
			line:SetSize(300, 22)
...
function window:CreateFrame5()
	...
	--> bars text
		...
		window:CreateLineBackground2 (frame5, "percentDropdown", "percentLabel", )
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> instances (delete instance - section Window Body)
...
-------- plugins
	...
	for absName, pluginObject in pairs (allplugins_toolbar) do 
		...
		local bframe = CreateFrame ("frame", "OptionsPluginToolbarBG", frame4,"BackdropTemplate")
	...
	for absName, pluginObject in pairs (allplugins_solo) do 
		...
		local bframe = CreateFrame ("frame", "OptionsPluginSoloBG", frame4,"BackdropTemplate")
		bframe:SetSize (640, 20)
		bframe:SetPoint ("topleft", frame4, "topleft", 10, y)
		bframe:SetBackdrop ({bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", tile = true, tileSize = 16, insets = {left = 1, right = 1, top = 0, bottom = 1}})
		bframe:SetBackdropColor (.3, .3, .3, .3)
	...
	if (InCombatLockdown()) then
		...
		if (not _detalhes.LoadingOptionsPanelFrame) then
			_detalhes.LoadingOptionsPanelFrame = CreateFrame ("frame", "LoadingOptionsPanelFrame", UIParent,"BackdropTemplate")
