...
mod:SetRevision("20200204005405")
...
mod:RegisterEventsInCombat(
	"SPELL_CAST_START 311176 316711 310184 310134 310130 317292 310331 315772 309698 310042 313400 308885 317066 319349 319350 319351 316970 318449 312782 316463",
	"SPELL_CAST_SUCCESS 315927 317102 317066 316970 318460",
	"SPELL_AURA_APPLIED 313334 308996 309991 313184 316541 316542 313793 315709 315710 312155 318196 318459 319309 319015 317112 319346 316711 318714",
...
--TODO, P3 spells with no detection like Stupefying Glare need scheduling for remaining modes
--New Voice: "leavemind" and "lowsanity"
--[[
(ability.id = 318449 or ability.id = 311176 or ability.id = 316711 or ability.id = 310184 or ability.id = 316463 or ability.id = 310134 or ability.id = 310130 or ability.id = 317292 or ability.id = 310331 or ability.id = 315772 or ability.id = 309698 or ability.id = 313400 or ability.id = 308885 or ability.id = 317066 or ability.id = 318196 or ability.id = 316970 or ability.id = 319351 or ability.id = 319350 or ability.id = 319349 or ability.id = 312782) and type = "begincast"
...
local warnCleansingProtocol					= mod:NewSpellAnnounce(316970, 2)
...
local timerEternalTormentCD					= mod:NewCDCountTimer(56.1, 318449, 311383, nil, nil, 2, nil, DBM_CORE_HEALER_ICON)--"Torment" short name
----Basher Tentacle
...
local timerEventHorizonCD					= mod:NewCDCountTimer(22.9, 318196, nil, nil, nil, 5, nil, DBM_CORE_TANK_ICON)--, nil, 2, 4
local timerAnnihilateCD						= mod:NewCDCountTimer(22.9, 318460, nil, nil, nil, 3)
...
local function stupefyingGlareLoop(self)
	...
	if self.Options.ArrowOnGlare then
		...
		if direction == "L" then
			DBM.Arrow:ShowStatic(90, 8)
		elseif direction == "R" then
			DBM.Arrow:ShowStatic(270, 8)
		end
	end
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif spellId == 316541 or spellId == 316542 then
		...
		if #ParanoiaTargets % 2 == 0 then
			...
			if not self:IsLFR() and args:IsPlayer() then--Only repeat yell on mythic and mythic+
